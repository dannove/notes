...
```
RotationWarperBase类继承自RotationWarper，它规定了其实例运用的投影方法projector_（**使用了模板类的方法**），projector_在buildMaps方法中负责正向和反向投影
``` C++
template <class P>
...
```
SphericalWarper类继承自RotationWarperBase类，规定了其投影方法为**SphericalProjector**。该类重写了buildMaps和detectResultRoi方法，以支持OPENCL加速。如果没有配置OPENCL，则仍调用父类的方法。
``` c++
